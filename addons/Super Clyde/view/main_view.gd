...
const DialogueSettings = preload("res://addons/Super Clyde/components/settings.gd")
...
func _ready() -> void:
	...
	close_confirmation_dialog.ok_button_text = Syntax.translate("confirm_close.save")
	close_confirmation_dialog.add_button(Syntax.translate("confirm_close.discard"), true, "discard")

	settings_view.editor_plugin = editor_plugin

	errors_dialog.dialog_text = Syntax.translate("errors_in_script")
...
func load_from_version_refresh(just_refreshed: Dictionary) -> void:
	...
	updated_dialog.dialog_text = Syntax.translate("update.success").format({ version = update_button.get_version() })
...
func close_file(file: String) -> void:
	...
	if buffer.text == buffer.pristine_text:
		...
		close_confirmation_dialog.dialog_text = Syntax.translate("confirm_close").format({ path = file.get_file() })
...
func apply_theme() -> void:
	if is_instance_valid(editor_plugin) and is_instance_valid(code_edit):
		...
		code_edit.theme_overrides = {
			...
			logic_color = editor_settings.get_setting("text_editor/theme/highlighting/function_color"),
		...
		new_button.tooltip_text = Syntax.translate("start_a_new_file")

		open_button.icon = get_theme_icon("Load", "EditorIcons")
		open_button.tooltip_text = Syntax.translate("open_a_file")

		save_all_button.icon = get_theme_icon("Save", "EditorIcons")
		save_all_button.tooltip_text = Syntax.translate("start_all_files")

		test_button.icon = get_theme_icon("PlayScene", "EditorIcons")
		test_button.tooltip_text = Syntax.translate("test_dialogue")

		search_button.icon = get_theme_icon("Search", "EditorIcons")
		search_button.tooltip_text = Syntax.translate("search_for_text")

		insert_button.icon = get_theme_icon("RichTextEffect", "EditorIcons")
		insert_button.text = Syntax.translate("insert")

		translations_button.icon = get_theme_icon("Translation", "EditorIcons")
		translations_button.text = Syntax.translate("translations")

		settings_button.icon = get_theme_icon("Tools", "EditorIcons")
		settings_button.tooltip_text = Syntax.translate("settings")

		docs_button.icon = get_theme_icon("Help", "EditorIcons")
		docs_button.text = Syntax.translate("docs")
		...
		popup.clear()
		popup.add_icon_item(get_theme_icon("RichTextEffect", "EditorIcons"), Syntax.translate("insert.wave_bbcode"), 0)
		popup.add_icon_item(get_theme_icon("RichTextEffect", "EditorIcons"), Syntax.translate("insert.shake_bbcode"), 1)
		popup.add_separator()
		popup.add_icon_item(get_theme_icon("Time", "EditorIcons"), Syntax.translate("insert.typing_pause"), 3)
		popup.add_icon_item(get_theme_icon("ViewportSpeed", "EditorIcons"), Syntax.translate("insert.typing_speed_change"), 4)
		popup.add_icon_item(get_theme_icon("DebugNext", "EditorIcons"), Syntax.translate("insert.auto_advance"), 5)
		popup.add_separator(Syntax.translate("insert.templates"))
		popup.add_icon_item(get_theme_icon("RichTextEffect", "EditorIcons"), Syntax.translate("insert.title"), 6)
		popup.add_icon_item(get_theme_icon("RichTextEffect", "EditorIcons"), Syntax.translate("insert.dialogue"), 7)
		popup.add_icon_item(get_theme_icon("RichTextEffect", "EditorIcons"), Syntax.translate("insert.response"), 8)
		popup.add_icon_item(get_theme_icon("RichTextEffect", "EditorIcons"), Syntax.translate("insert.random_lines"), 9)
		popup.add_icon_item(get_theme_icon("RichTextEffect", "EditorIcons"), Syntax.translate("insert.random_text"), 10)
		popup.add_separator(Syntax.translate("insert.actions"))
		popup.add_icon_item(get_theme_icon("RichTextEffect", "EditorIcons"), Syntax.translate("insert.jump"), 11)
		popup.add_icon_item(get_theme_icon("RichTextEffect", "EditorIcons"), Syntax.translate("insert.end_dialogue"), 12)

		# Set up the translations menu
		popup = translations_button.get_popup()
		popup.clear()
		popup.add_icon_item(get_theme_icon("Translation", "EditorIcons"), Syntax.translate("generate_line_ids"), TRANSLATIONS_GENERATE_LINE_IDS)
		popup.add_separator()
		popup.add_icon_item(get_theme_icon("FileList", "EditorIcons"), Syntax.translate("save_characters_to_csv"), TRANSLATIONS_SAVE_CHARACTERS_TO_CSV)
		popup.add_icon_item(get_theme_icon("FileList", "EditorIcons"), Syntax.translate("save_to_csv"), TRANSLATIONS_SAVE_TO_CSV)
		popup.add_icon_item(get_theme_icon("AssetLib", "EditorIcons"), Syntax.translate("import_from_csv"), TRANSLATIONS_IMPORT_FROM_CSV)
...
func build_open_menu() -> void:
	...
	menu.clear()
	menu.add_icon_item(get_theme_icon("Load", "EditorIcons"), Syntax.translate("open.open"), OPEN_OPEN)
	menu.add_separator()

	var recent_files = DialogueSettings.get_recent_files()
	if recent_files.size() == 0:
		menu.add_item(Syntax.translate("open.no_recent_files"))
	...
	menu.add_separator()
	menu.add_item(Syntax.translate("open.clear_recent_files"), OPEN_CLEAR)
...
func show_build_error_dialog() -> void:
	build_error_dialog.dialog_text = Syntax.translate("errors_with_build")
...
func export_translations_to_csv(path: String) -> void:
	...
	var dialogue: Dictionary = ClydeParser.parse_string(code_edit.text, current_file_path).lines
	...
	for key in dialogue.keys():
		...
		if not line.type in [Syntax.TOKEN_TEXT]: continue
...
func export_character_names_to_csv(path: String) -> void:
	...
	var character_names: PackedStringArray = ClydeParser.parse_string(code_edit.text, current_file_path).character_names
...
func _on_docs_button_pressed() -> void:
	OS.shell_open("https://github.com/PaganSeaWitch/Super-Clyde-Godot-Plugin/blob/master/LANGUAGE.md")
...
func _on_files_popup_menu_about_to_popup() -> void:
	...
	files_popup_menu.add_item(Syntax.translate("buffer.save"), ITEM_SAVE, KEY_MASK_CTRL | KEY_MASK_ALT | KEY_S)
	files_popup_menu.add_item(Syntax.translate("buffer.save_as"), ITEM_SAVE_AS)
	files_popup_menu.add_item(Syntax.translate("buffer.close"), ITEM_CLOSE, KEY_MASK_CTRL | KEY_W)
	files_popup_menu.add_item(Syntax.translate("buffer.close_all"), ITEM_CLOSE_ALL)
	files_popup_menu.add_item(Syntax.translate("buffer.close_other_files"), ITEM_CLOSE_OTHERS)
	files_popup_menu.add_separator()
	files_popup_menu.add_item(Syntax.translate("buffer.copy_file_path"), ITEM_COPY_PATH)
	files_popup_menu.add_item(Syntax.translate("buffer.show_in_filesystem"), ITEM_SHOW_IN_FILESYSTEM)
