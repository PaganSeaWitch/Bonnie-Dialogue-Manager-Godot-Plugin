...
const DEFAULT_SETTINGS = {
	...
	"custom_test_scene_path" = "res://addons/Super Cylde/test_scene.tscn"
}
...
static func prepare() -> void:
	...
	for key in [
		...
		if ProjectSettings.has_setting("Super Clyde/%s" % key):
			var value = ProjectSettings.get_setting("Super Clyde/%s" % key)
			ProjectSettings.set_setting("Super Clyde/%s" % key, null)
			set_setting(key, value)
	
	# Set up defaults
	for setting in DEFAULT_SETTINGS:
		if ProjectSettings.has_setting("Super Clyde/general/%s" % setting):
			ProjectSettings.set_initial_value("Super Clyde/general/%s" % setting, DEFAULT_SETTINGS[setting])
	ProjectSettings.save()


static func set_setting(key: String, value) -> void:
	ProjectSettings.set_setting("Super Clyde/general/%s" % key, value)
	ProjectSettings.set_initial_value("Super Clyde/general/%s" % key, DEFAULT_SETTINGS[key])
	ProjectSettings.save()


static func get_setting(key: String, default):
	if ProjectSettings.has_setting("Super Clyde/general/%s" % key):
		return ProjectSettings.get_setting("Super Clyde/general/%s" % key)
	else:
		return default
...
static func get_user_config() -> Dictionary:
	...
	if FileAccess.file_exists(Syntax.USER_CONFIG_PATH):
		var file: FileAccess = FileAccess.open(Syntax.USER_CONFIG_PATH, FileAccess.READ)
...
static func save_user_config(user_config: Dictionary) -> void:
	var file: FileAccess = FileAccess.open(Syntax.USER_CONFIG_PATH, FileAccess.WRITE)
