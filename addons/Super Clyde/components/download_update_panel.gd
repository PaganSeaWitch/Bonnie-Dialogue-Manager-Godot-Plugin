...
var next_version_release: Dictionary:
	set(value):
		...
		label.text = Syntax.translate("update.is_available_for_download") % value.tag_name.substr(1)
	get:
		return next_version_release


func _ready() -> void:
	$VBox/Center/DownloadButton.text = Syntax.translate("update.download_update")
	$VBox/Center2/NotesButton.text = Syntax.translate("update.release_notes")
...
func _on_download_button_pressed() -> void:
	...
	download_button.text = Syntax.translate("update.downloading")
...
func _on_http_request_request_completed(result: int, response_code: int, headers: PackedStringArray, body: PackedByteArray) -> void:
	...
	if DirAccess.dir_exists_absolute("res://addons/Super Clyde"):
		DirAccess.remove_absolute("res://addons/Super Clyde")
