...
const REMOTE_RELEASES_URL = "https://api.github.com/repos/PaganSeaWitch/Super-Clyde-Godot-Plugin/releases"
const LOCAL_CONFIG_PATH = "res://addons/Super Clyde/plugin.cfg"
...
func _on_http_request_request_completed(result: int, response_code: int, headers: PackedStringArray, body: PackedByteArray) -> void:
	...
	if version_to_number(next_version) > version_to_number(current_version):
		...
		text = Syntax.translate("update.available").format({ version = next_version })
		show()
...
func _on_download_update_panel_updated(updated_to_version: String) -> void:
	...
	needs_reload_dialog.dialog_text = Syntax.translate("update.needs_reload")
	needs_reload_dialog.ok_button_text = Syntax.translate("update.reload_ok_button")
	needs_reload_dialog.cancel_button_text = Syntax.translate("update.reload_cancel_button")
	needs_reload_dialog.popup_centered()
	
	needs_reload = true
	text = Syntax.translate("update.reload_project")
	apply_theme()


func _on_download_update_panel_failed() -> void:
	download_dialog.hide()
	update_failed_dialog.dialog_text = Syntax.translate("update.failed")
