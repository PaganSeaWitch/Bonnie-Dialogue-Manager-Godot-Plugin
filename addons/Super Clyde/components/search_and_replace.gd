...
var result_index: int = -1:
	set(next_result_index):
		...
		result_label.text = Syntax.translate("n_of_n").format({ index = result_index + 1, total = results.size() })
	get:
		return result_index
...
func _ready() -> void:
	...
	previous_button.tooltip_text = Syntax.translate("search.previous")
	next_button.tooltip_text = Syntax.translate("search.next")
	match_case_button.text = Syntax.translate("search.match_case")
	$Search/ReplaceCheckButton.text = Syntax.translate("search.toggle_replace")
	replace_button.text = Syntax.translate("search.replace")
	replace_all_button.text = Syntax.translate("search.replace_all")
	$Replace/ReplaceLabel.text = Syntax.translate("search.replace_with")
