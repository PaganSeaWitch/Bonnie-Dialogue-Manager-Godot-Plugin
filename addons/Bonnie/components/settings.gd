...
static func prepare() -> void:
	...
	for key in [
		...
		if ProjectSettings.has_setting("Bonnie/%s" % key):
			var value = ProjectSettings.get_setting("Bonnie/%s" % key)
			ProjectSettings.set_setting("Bonnie/%s" % key, null)
			set_setting(key, value)
	
	# Set up defaults
	for setting in DEFAULT_SETTINGS:
		if ProjectSettings.has_setting("Bonnie/general/%s" % setting):
			ProjectSettings.set_initial_value("Bonnie/general/%s" % setting, DEFAULT_SETTINGS[setting])
	ProjectSettings.save()


static func set_setting(key: String, value) -> void:
	ProjectSettings.set_setting("Bonnie/general/%s" % key, value)
	ProjectSettings.set_initial_value("Bonnie/general/%s" % key, DEFAULT_SETTINGS[key])
	ProjectSettings.save()


static func get_setting(key: String, default):
	if ProjectSettings.has_setting("Bonnie/general/%s" % key):
		return ProjectSettings.get_setting("Bonnie/general/%s" % key)
	else:
		return default
